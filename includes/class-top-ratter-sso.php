...
//            echo $wpdb->last_error;
    ...
    public function handle_unlink_characters_submit(){
       ...
       if(!$hasmainchar){
          ...
          if($haschars){
              $wpdb->update( $wpdb->prefix.'tr_users_chars', array('is_main_char'=>'1'), array('uc_id'=>$haschars['uc_id']));
          }
       }
    }
    /**
    ...
    public function handle_edit_SSO_credentials(){
        ...
        if ($errors) {
            ...
            $where=array('id'=>$_POST ['edit_credentials_id']);
                ...
                if (isset ( $_GET['code'] )) {
                    ...
                    if($state=="SSSStateClient_23egy34DFHJ5y"){
                        ...
                        if($statequery['corporation_id']==''){
                            ...
                            $data=array('corporation_id'=>$char_corp['corporation_id']);
                            $where=array('id'=>$statequery['id']);
...
//                     echo'<p>Looks like the token was added successfuly.</p>';
     ...
     *
     * @param $auth_code authentication code recieved in previous steps
     *
    ...
    public function basic_curl_eve_sso_auth_token($auth_code){
        ...
        $postdata = ['grant_type' => "authorization_code", 'code' => $auth_code];
     ...
     *
     * @param $owner_id int api character owner id from table kur_tr_characters
     * @param $token string valid working token.
     *
     * @return bolean 
     */
     ...
     *
     * @param $char_id  int id of the character from tr_characters table, NOT OWNER ID
     *
...
//                     echo'TRANSIENT- invalid grant:'.$res['error_description'];
                    ...
                    //unlink character
                    $wpdb->delete( $wpdb->prefix . 'tr_users_chars', array('user_id'=>$user->ID,'char_id'=>$token_data['character_id']));
                    
                    // delete the token.
                    $wpdb->delete( $wpdb->prefix . 'tr_sso_tokens', array('character_id'=>$token_data['character_id']));
...
//             echo 'token fresh';
     ...
     *
     * @param $refresh_token  string refresh token from table
     *
     ...
     * 
     * @param $data Array associative array containing response of the eve token exchange result.
     *
     * @return null
     */
     ...
     *
     * @param $char_pub_data Array associative array containing data from character public data api.
     *
    ...
    /**
     * Attaches character to the logged in user. Also checks if this is main or alt character.
     *
     * @param $char_id  int id of the character from tr_characters table, NOT OWNER ID
     *
     * @return null
   ...
   public function user_login_token_check() {
       ...
       if($users_chars){
           ...
           $sql = "SELECT * FROM `" . $wpdb->prefix . "tr_sso_credentials`;";
           $officer_credentials = $wpdb->get_row ( "$sql", ARRAY_A );

           foreach($users_chars as $character){
               //get fresh token  and unlink if there is problem.
               ...
               if($token){
                   ...
                   if($char_corp['corporation_id']==$officer_credentials['corporation_id']){
                       // good, let this character pass
                   }else{
                       //unlink the character from user
                       $wpdb->delete( $wpdb->prefix . 'tr_users_chars', array('user_id'=>$current_user->ID,'char_id'=>$character['char_id']));
                   }
               } 
           }
       }
...
//                          $fresh_officer_token=$token;
     ...
     * 
     * @param $fresh_officer_token string a valid officer token
     * @param $corp_id int  corporation id.
     * @param $ref_id float reference id that is used to read older records of API 
     *
    ...
    public function corp_wallet_journal_API_call($fresh_officer_token,$corp_id,$ref_id=null){
        ...
        $header = array("Authorization: Bearer ".$fresh_officer_token);
